...
type UserQuery struct {
	...
	withClientSessions *ClientSessionQuery
...
// QueryClientSessions chains the current query on the "client_sessions" edge.
func (_q *UserQuery) QueryClientSessions() *ClientSessionQuery {
	query := (&ClientSessionClient{config: _q.config}).Query()
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withClientSessions: _q.withClientSessions.Clone(),
...
// WithClientSessions tells the query-builder to eager-load the nodes that are connected to
// the "client_sessions" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithClientSessions(opts ...func(*ClientSessionQuery)) *UserQuery {
	query := (&ClientSessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withClientSessions = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withClientSessions; query != nil {
		if err := _q.loadClientSessions(ctx, query, nodes,
			func(n *User) { n.Edges.ClientSessions = []*ClientSession{} },
			func(n *User, e *ClientSession) { n.Edges.ClientSessions = append(n.Edges.ClientSessions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadClientSessions(ctx context.Context, query *ClientSessionQuery, nodes []*User, init func(*User), assign func(*User, *ClientSession)) error {
	...
	query.withFKs = true
	query.Where(predicate.ClientSession(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ClientSessionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_client_sessions
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_client_sessions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_client_sessions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
